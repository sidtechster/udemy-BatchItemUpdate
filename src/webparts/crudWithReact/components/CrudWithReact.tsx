...
import { ISPHttpClientOptions, SPHttpClient, SPHttpClientResponse, SPHttpClientBatch } from '@microsoft/sp-http';
...
export default class CrudWithReact extends React.Component<ICrudWithReactProps, ICrudWithReactState> {
  ...
  public btnAddAndLoadInOneGo_click(): void {
    ...
    .then((listItems: IListItem[]) => {
      this.setState({ ListItems: listItems, status: "Added and loaded in one batch" });
    });
  }

  public createAndLoadInOneGo(): Promise<IListItem[]> {
    ...
    let promise: Promise<IListItem[]> = new Promise<IListItem[]>((resolve, reject) => {
      ...
      const batchOperations: Promise<IListItem | IListItem[]>[] = [
  ...
  private _getListItemsAsBatch(clientBatch: SPHttpClientBatch): Promise<IListItem[]> {
    const url: string = this.props.siteUrl + "/_api/web/lists/getbytitle('CarInventory')/items";
  ...
  public _addListItemsAsBatch(clientBatch: SPHttpClientBatch): Promise<IListItem> {
    let promise: Promise<IListItem> = new Promise<IListItem>((resolve, reject) => {
      const url: string = this.props.siteUrl + "/_api/web/lists/getbytitle('CarInventory')/items";
